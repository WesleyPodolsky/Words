...
class Topop(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.Surface((OBSTACLE_WIDTH, OBSTACLE_HEIGHT))
        self.image.fill('lightgreen')
    ...
    while True:
        while game_over == False:
            ...
            if pygame.time.get_ticks() - last_obstacle_time > 500:
                ...
                obstacle_count += add_obstacletwo(topops)
            ...
            collidertwo = pygame.sprite.spritecollide(player, topops, dokill=False)
            if collidertwo:
                collidertwo[0].explode()
        
            # Draw everything
            screen.fill("lightblue")
            grass = pygame.Rect(0,270,10000,10000)
            pygame.draw.rect(screen, 'lightgreen', grass)
            ...
            if passedobj > hiscore:
                hiscore = passedobj
            obstacle_text = font.render(f" highscore: {hiscore}", True, 'darkblue')
...
class Button():
    def __init__(self, butx, buty, butwid, buthigh, butcolor, buttext, buttxtcolor):
        ...
        self.butwid = 60
        self.buthigh = 400
        self.butcolor = "lightgreen"
        self.buttext = 'u died   --> retry'
        self.buttxtcolor = 'darkgreen'
